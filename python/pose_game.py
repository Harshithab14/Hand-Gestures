...
def checkLeftRight(image, results, draw=False, display=False):
    ...
    left_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_INDEX].x * width)

    # Retreive the x-corrdinate of the right shoulder landmark.
    right_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_INDEX].x * width)
...
def checkJumpCrouch(image, results, MID_Y=150, draw=False, display=False):
    '''
    ...
    if draw:
        ...
        #cv2.line(output_image, (0, 2*(MID_Y/3)),(width, 2*(MID_Y/3)),(255, 255, 255), 2)
